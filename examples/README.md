...
descriptor `.length()`, and source-backed element Drop cleanup on the default compiler path.
...
| `dynamic_array_owned_parameter.or` | owned-element `DynamicArray<Payload>` parameter transfer, descriptor `.length()`, and source-backed element Drop cleanup | backend |
...
`dynamic_array_owned_parameter.or` for owned growable-sequence parameter length, transfer, and callee cleanup coverage. Use
