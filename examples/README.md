...
`dynamic_array_owned_parameter.or` demonstrates owned-element `DynamicArray<Payload>` parameter transfer with
source-backed element Drop cleanup on the default compiler path.
...
`local_result_array_payload_choice_switch.or` demonstrates distinct-payload tagged buffer recovery for fixed-array
...
`local_result_multi_payload_choice_branch_flow.or` demonstrates multi-payload choice values produced by final
ternary and `switch` branches, then consumed through function calls and payload-pattern `switch` recovery.
`local_result_multi_payload_choice_reassignment.or` demonstrates mutable multi-payload choice reassignment across
`if` and `switch` branches before payload-pattern recovery.
`local_result_multi_payload_choice_record_field.or` demonstrates multi-payload choice values stored in record fields,
including record construction, field reassignment, and payload-pattern recovery through field access.
`local_result_multi_payload_choice_array_element.or` demonstrates multi-payload choice values stored in fixed-array
...
`local_result_multi_payload_choice_nested_array_record.or` demonstrates multi-payload choice values in nested
fixed-array record elements, including multi-level indexed field assignment and recovery through indexed field reads.
...
named iteration, computed helper-returned View iteration, and method-returned View iteration through scalar and
member-derived receivers.
...
| `dynamic_array_owned_parameter.or` | owned-element `DynamicArray<Payload>` parameter transfer with source-backed element Drop cleanup | backend |
...
| `tour_05_bindings_operators.or` | `let`, `var`, assignment, arithmetic/modulo, negation, boolean, ternary, bitwise, shifts | backend |
...
| `local_result_multi_payload_choice_branch_flow.or` | generic `Result<UInt32>` multi-payload ternary/switch branch production plus downstream consumption | backend |
| `local_result_multi_payload_choice_reassignment.or` | mutable generic `Result<UInt32>` multi-payload reassignment across `if`/`switch` branches plus downstream consumption | backend |
...
| `local_result_multi_payload_choice_nested_array_record.or` | generic `Result<UInt32>` multi-payload nested fixed-array record elements, multi-level indexed field assignment, and downstream indexed field consumption | backend |
...
| `view_descriptor_reads.or` | `shared.View<T>`/`exclusive.View<T>` descriptor length, checked index reads, exclusive indexed assignment, named iteration, computed helper-returned View iteration, and method-returned View iteration through scalar and member-derived receivers | backend |
...
`dynamic_array_owned_parameter.or` for owned growable-sequence parameter transfer and callee cleanup coverage. Use
...
Run the scalar DynamicArray parameter demo with:

```sh
build/tools/orisonc/orisonc --emit-llvm examples/dynamic_array_parameter_reads.or
```

It validates scalar `DynamicArray<UInt32>` parameter descriptors, cleanup, `.length()`, checked indexing, and
descriptor iteration on the default compiler path.
