...
- `compiler/syntax/` — lexer and early parser infrastructure for module headers, block structure, signature syntax, basic statements, and minimal expression trees
...
This repository currently contains the initial language design documents plus an early native `C++23` frontend scaffold that can read a source file, lex a token stream, parse indentation-based blocks, build minimal signature/type/statement/expression syntax, and report explicit diagnostics.
