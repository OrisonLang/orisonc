...
- first semantic validation for concurrency surface rules: `await` now produces a frontend diagnostic when used outside `async` functions or methods
...
- 2026-04-26: completed the first semantic concurrency check so `await` is rejected outside `async` functions, extended smoke coverage, and synchronized the parity records.
