...
- 2026-04-28: extended constructor-pattern `switch` arm semantics through nested subpatterns and added provisional shape checks, so payload names bind recursively while unsupported payload expressions like `head + 1` now produce semantic diagnostics.
