...
- statement parsing for `let`, `var`, plain `=` assignment, `return`, `break`, `continue`, expression statements, inline and block-arm `switch`, `guard ... else`, `if` with an optional `else` block, `while`, `repeat ... while`, `for ... in`, and block `defer`
...
- additional statement forms from the updated docs, including block `unsafe`
...
- 2026-04-26: completed parser support for `repeat ... while` statements, extended smoke coverage, and synchronized the parity records.
