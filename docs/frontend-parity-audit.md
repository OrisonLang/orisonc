...
- expression parsing for names, decimal/hex/binary integer literals, string literals, boolean literals, array literals, unary `-`, `not`, and `bit_not`, calls, member access, index access, ternary `?:`, and binary `+`, `-`, `*`, `%`, `/`, `==`, `!=`, `<`, `<=`, `>`, `>=`, `and`, `or`, `bit_and`, `bit_or`, `bit_xor`, `shift_left`, and `shift_right`
...
- richer expression, literal, and pattern grammar beyond the current narrow subset, including constructor patterns and `?.`
...
- 2026-04-26: completed parser support for ternary expressions, extended smoke coverage, and synchronized the parity records.
