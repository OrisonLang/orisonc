...
- 2026-04-29: tightened placeholder `Pointer(...)` validation so current pointer-constructor calls now require both an active `unsafe` boundary and exactly one source argument, standing in for future typed `Pointer<T>(Address)` checking.
