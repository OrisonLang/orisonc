...
- 2026-09-11: Computed DynamicArray unsupported-shape diagnostics now include a short source-oriented summary ahead of
  the detailed audit chain. Existing unsupported-shape proof reports remain intact for parity checks.
- 2026-09-11: Computed DynamicArray same-owner cleanup-proof failures now include a short source-oriented summary ahead
  of the detailed audit chain, and owned-element cleanup-authorization failures append a short source-oriented summary.
...
- 2026-09-10: DynamicArray cleanup capability and readiness pair fields now use owned-cleanup pair names internally.
  Existing `element-drop-pairs` and `missing-element-drop-pairs` report text remains compatibility output.
...
- 2026-09-10: Semantic cleanup report orchestration now uses owned-cleanup source filenames and helper names
  internally. CLI flags, rendered report text, and drop-readiness source-correlation naming remain compatibility
  surfaces.
...
- 2026-08-29: Computed DynamicArray ownership planning now unwraps source-proven descriptor-forwarding helper calls.
  ...
  host execution.
- 2026-08-29: `tests/fixtures/dynamic_array_returned_alias_chain_owned_computed_for_cleanup_run.or` now pins a
  returned `DynamicArray<Payload>` moved through two local aliases before return, then consumed by same-owner computed
  iteration with cleanup, object emission, build, and host execution.
- 2026-08-29: `tests/fixtures/dynamic_array_returned_choice_payload_owned_computed_for_cleanup_run.or` now pins a
  returned choice carrying a `DynamicArray<Payload>` payload, caller-side switch payload binding, computed iteration,
...
- 2026-08-25: lowering smoke now pins the internal post-merge negative path for the same preexisting local cleanup
  shape: after final `if`/`switch` cleanup ownership merges, reading `items` reports `use after move: items`.
...
- 2026-08-26: Final-if branch-local cleanup composition now covers nested returned aggregate-field owners.
...
- 2026-08-26: Forwarded final-if nested returned-field cleanup now has paired owner-reuse coverage. Reusing
...
- 2026-08-26: Mixed direct/helper-forwarded branch joins now cover nested returned aggregate-field cleanup. One branch
...
- 2026-08-27: Mixed direct/helper-forwarded switch cases now feed final-if and final-switch direct returned-field
  cleanup composition. The checked `PayloadBox` paths join direct and `forward_box` cases, clean `returned.values`,
  then local `scratch`, suppress stale `box.values` cleanup, and reject `scratch` reuse.
...
- 2026-08-27: Direct lowering fixtures now construct semantic-summary DynamicArray descriptor facts directly, replacing
...
- 2026-08-28: Shared production pipeline defaults now enable DynamicArray construction/index/append/cleanup and
  computed `for` gates. Source Drop lowering remains separately controlled.
...
- 2026-08-28: Nested returned aggregate cleanup now has production-default run coverage with explicit source Drop
  ...
  record.
- 2026-08-28: Source Drop production-default trial exposed a double free in
...
- 2026-08-30: Straight helper-forwarded returned nested aggregate-field final-if/final-switch cleanup fixtures now have
...
- 2026-08-30: Owned-result direct, returned-local, and nested final-if/final-switch `DynamicArray` cleanup fixtures now
...
- 2026-09-09: Positive choice-payload and generic DynamicArray fixtures no longer declare `interface Drop` or
...
- 2026-09-09: Forwarded returned aggregate-field DynamicArray rejection fixtures no longer declare `interface Drop` or
  `implements Drop`. Array CLI and pipeline smoke coverage verify compiler-derived cleanup preserves returned-field
  post-move reuse, final-control branch-local reuse, helper owner-mismatch, and unsupported-shape diagnostics.
...
- 2026-09-10: Receiver/member-chain DynamicArray rejection fixtures no longer declare `interface Drop` or
...
- 2026-09-12: Computed DynamicArray cleanup-owner-unproven coverage now reaches the production lowering boundary
  through an internal proof-suppression fixture, keeping the defensive diagnostic pinned without treating it as a
