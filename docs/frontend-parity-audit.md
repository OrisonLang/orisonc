...
- 2026-04-29: tightened placeholder `Pointer(...)` validation again so current pointer-constructor calls now require an active `unsafe` boundary, exactly one source argument, and a structurally address-like source operand, standing in for future typed `Pointer<T>(Address)` checking.
