...
- lexing for package, record, function, let, var, return, switch, default, guard, if, else, and while keywords
...
- statement parsing for `let`, `var`, `return`, expression statements, inline-arm `switch`, `guard ... else`, `if` with an optional `else` block, and `while`
...
- multi-line switch-arm block bodies, `for`, and `defer` statement parsing
...
- 2026-04-23: completed parser support for `while`, extended smoke coverage, and synchronized the parity records.
