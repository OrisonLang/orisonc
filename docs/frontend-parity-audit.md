...
- 2026-04-28: tightened constructor-pattern `switch` arm semantics again so declared choice variants now also enforce payload arity, with diagnostics for cases like `Node(head)` and `Empty(value)` in addition to the existing shape and duplicate-binding checks.
