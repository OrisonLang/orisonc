...
- 2026-04-29: added structural unsafe-intrinsic operand validation, so `address_of` now requires addressable storage and `raw_read`, `raw_write`, `raw_offset`, `volatile_read`, and `volatile_write` now require structurally address-like first operands in addition to active `unsafe` boundaries.
