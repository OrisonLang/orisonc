...
- lexing for package, record, function, let, var, return, switch, default, guard, if, and else keywords
...
- statement parsing for `let`, `var`, `return`, expression statements, inline-arm `switch`, `guard ... else`, and `if` with an optional `else` block
...
- multi-line switch-arm block bodies, `while`, `for`, and `defer` statement parsing
...
- 2026-04-23: completed parser support for inline-arm `switch`, added `=>`, `default`, `<=`, and `>=` tokenization/parsing, extended smoke coverage, and synchronized the parity records.
