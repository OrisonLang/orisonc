...
- 2026-04-29: added placeholder unsafe pointer-construction validation, so current `Pointer(...)` constructor calls now also require an active `unsafe` function or `unsafe` block alongside the existing unsafe call and raw/MMIO intrinsic checks.
