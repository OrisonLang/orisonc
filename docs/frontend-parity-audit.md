...
- first semantic validation for concurrency surface rules: `await` and `task` now produce frontend diagnostics when used outside `async` functions or methods, `task`/`thread` bodies must end in a value-producing statement shape, and concurrency bodies now reject captures of mutable outer locals and receiver `this`
...
- 2026-04-26: extended the first semantic concurrency pass so concurrency bodies now reject receiver `this` captures, extended smoke coverage, and synchronized the parity records.
