...
- 2026-04-28: added semantic case-local bindings for constructor-pattern `switch` arms, so payload names like `Node(head, tail)` now shadow outer locals inside the arm while top-level name patterns remain non-binding.
