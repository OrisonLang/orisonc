...
- 2026-04-28: extended structural provenance through loop-shaped state too, so `while` and `for` now merge the zero-iteration and body-executed paths while `repeat` preserves provenance established by its guaranteed first body execution.
