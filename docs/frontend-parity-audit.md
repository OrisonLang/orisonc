...
- first semantic validation for concurrency surface rules: `await` and `task` now produce frontend diagnostics when used outside `async` functions or methods
...
- 2026-04-26: extended the first semantic concurrency pass so both `await` and `task` are rejected outside `async` functions, extended smoke coverage, and synchronized the parity records.
