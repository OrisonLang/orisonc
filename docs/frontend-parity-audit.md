...
- 2026-04-29: added placeholder pointer-typed binding and return validation, so explicit `Pointer<...>` annotations now also require structurally pointer-like initializer and return expressions on top of the existing `Pointer(...)` constructor checks.
