...
- lexing for package, import, type, record, choice, interface, implements, function, public, private, from, as, shared, exclusive, let, var, return, switch, default, guard, if, else, while, for, in, and defer keywords
- indentation-sensitive block structure with explicit `indent` and `dedent` tokens
- package declarations, import blocks, type aliases, record declarations, choice declarations, interface declarations, `implements` declarations, and function headers
...
- parsed interface method signatures including generic interface parameter lists and `implements Interface for Type` method blocks
...
- CLI parse output for import/type/choice/interface/implementation counts, declaration visibility, implementation targets, and first-statement nested/alternate/switch-arm counts

### Pending

- additional top-level declarations and blocks from the updated docs, including `extend` and `where`
...
- 2026-04-25: completed parser support for `implements` declarations including method blocks, extended smoke coverage, and synchronized the parity records.
