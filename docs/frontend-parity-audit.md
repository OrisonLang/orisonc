...
- 2026-04-28: extended provenance merging from assignments and ternaries into `if`/`else` and `switch` branch shaping, so same-origin reassignments inside control-flow statements now preserve the same await/join/return structural checks.
