...
- 2026-04-29: added unsafe-call propagation checks, so calling a declared `unsafe` function or method now also requires an active `unsafe` function or `unsafe` block in addition to the existing raw/MMIO intrinsic checks.
