...
- 2026-04-29: deepened the shallow low-level type pass so `raw_read` now exposes pointee types and `raw_write` now checks pointee/value mismatches when `Pointer<T>` is already known, further reducing reliance on raw expression shape.
