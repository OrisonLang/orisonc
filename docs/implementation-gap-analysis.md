...
- Diagnostics are useful. Runtime-index splice-conflict blockers now include the conflicting source lines,
  ...
  member-cleanup helper binding, production-readiness, and production blocker diagnostics now include source-line and
  source-text metadata when the originating constructor move is known, and promotion blocker diagnostics now carry
  source-line/source-text metadata from the gate or matched readiness record. Promotion checklist and seam reports now
  carry source-line/source-text metadata when the originating constructor move is known. Typed promotion gate and
  execution summary reports now carry the same source correlation. Other lowering and cleanup failures still need
  richer source-span correlation.
