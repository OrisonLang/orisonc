...
- Diagnostics are useful. Runtime-index splice-conflict blockers now include the conflicting source lines,
  ...
  the originating constructor move is known. Typed promotion gate and execution summary reports now carry the same
  source correlation. Other lowering and cleanup failures still need richer source-span correlation.
