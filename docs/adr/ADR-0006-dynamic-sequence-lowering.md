...
- Loop-body owned transfers are preserved after loop exit for parameter cleanup safety. Production CLI coverage now
  ...
  `repeat` loop body.
- Loop-control paths preserve the same owned-parameter replacement and move-safety rules. Production CLI coverage now
  verifies owned-element replacement before terminal `break` and `continue`, and rejects post-loop reads after moving
  an owned `DynamicArray<T>` parameter before either loop-control transfer.
...
- Production pipeline defaults now also enable DynamicArray construction/index/append/cleanup and computed `for` gates.
  Source Drop lowering remains explicitly controlled while its cleanup-shape migration matures.
...
- `tests/fixtures/dynamic_array_switch_returned_owned_computed_for_cleanup_run.or` now pins switch-returned
  owned-element descriptors joined by a multi-case `switch` result and consumed by nested same-owner computed
  iteration, including PHI return, final-use Drop walks, deallocation, descriptor finalization, and executable
  coverage.
...
- Multi-nested final `switch` cleanup now supports consuming a branch-local scratch owner in one nested case while
  cleaning sibling branch-local scratch owners before their joins. Branch-local cleanup merge state now drops consumed
  entries for cleanup-bearing source bindings introduced inside the branch/case scope.
...
- Branch-consumer ternary cleanup now supports final consumer helpers that move the selected owner through a nested
  local alias chain and return the final alias, while suppressing stale cleanup for the parameter and each
  intermediate alias.
...
- Branch-consumer ternary cleanup now supports final consumer helpers that store same-type helper-call results in
  another local before returning, with stale cleanup suppressed for the stored returned owner.
...
- The direct final-if composition now has paired branch-local reuse coverage. Passing same-arm `scratch` to an
  owned-parameter helper twice after the `returned.values` computed loop fails with `use after move: scratch`.
...
- Switch-returned direct aggregate fields now cover helper-forwarded switch cases. The checked `PayloadBox` switch join
  calls `forward_box`, cleans caller-owned `returned.values`, suppresses stale `box.values` cleanup, and rejects
  post-cleanup `returned.values` reuse.
...
- Mixed direct/helper-forwarded switch cases now feed final-if and final-switch direct returned-field cleanup
  composition. The checked `PayloadBox` paths join direct and `forward_box` cases, clean `returned.values`, then local
  `scratch`, suppress stale `box.values` cleanup, and reject `scratch` reuse.
...
- Runtime-index member-cleanup typed promotion gate reports now carry source-line metadata and trimmed source text when
...
- Branch-mixed-forwarded, switch-forwarded, and switch-mixed-forwarded returned aggregate-field and nested
  aggregate-field final-if/final-switch cleanup fixtures now have generic CLI production coverage.
- Straight helper-forwarded returned nested aggregate-field final-if/final-switch cleanup fixtures now have generic CLI
  production coverage.
- Choice-payload switch/final-switch binding owned-computed cleanup fixtures now have generic CLI production coverage.
...
- Owned-result direct nested-if/switch and mixed if-switch/switch-if branch cleanup fixtures now have generic CLI
  production coverage.
...
- Returned aggregate-field and nested aggregate-field final-if/final-switch cleanup fixtures now have generic CLI
  production coverage.
- Switch-returned base, aggregate-field, and nested aggregate-field computed/final-control cleanup fixtures now have
...
- Returned aggregate receiver coverage now includes fixed arrays of records that contain choice payload DynamicArray
...
- Returned aggregate receiver cleanup now registers `Maybe<T>` sibling fields with DynamicArray descendants as
  ...
  `Some` payloads.
- Returned aggregate receiver coverage now includes fixed arrays of `Maybe<DynamicArray<T>>` sibling payloads,
  proving finite array traversal composes with Maybe tag-guarded descriptor cleanup.
- Returned aggregate receiver coverage now includes nested fixed arrays of `Maybe<DynamicArray<T>>` sibling payloads,
...
- Returned aggregate receiver coverage now includes `Maybe<Array<Array<DynamicArray<T>, N>, M>>` sibling payloads,
  proving Maybe payload cleanup can descend through multidimensional finite arrays of descriptors.
- Returned aggregate receiver coverage now includes `Maybe<Array<RecordWithDynamicArray, N>>` sibling payloads,
  proving Maybe payload cleanup composes finite array traversal with nested record-field descriptor extraction.
...
- Extend returned aggregate sibling cleanup beyond finite record-field, fixed-array, choice-sibling, choice record
  payload, choice fixed-array record payload, choice nested fixed-array record payload, fixed-array choice-sibling,
  nested fixed-array choice-sibling, direct `Maybe` payload, fixed-array `Maybe` payload, and
