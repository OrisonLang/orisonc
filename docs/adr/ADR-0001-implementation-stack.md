...
- Lowering context now collects record field layout metadata from parsed records, preserving field order, source type
  ...
  `pointer.rows[index][inner] = value`, and branch-local or switch-local nested field stores read after the
  control-flow join. CLI smoke coverage pins user-facing LLVM extraction shapes for mixed inferred
