...
- Lowering context now also synthesizes monomorphized record layout entries for concrete generic record types
  ...
  after ordinary source record layouts. This enables the first concrete generic record constructor lowering path and
  field reads from annotated immutable aggregate locals while broader generic aggregate interactions remain follow-up
  work.
