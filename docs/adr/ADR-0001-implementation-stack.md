...
- Lowering context now collects record field layout metadata from parsed records, preserving field order, source type
  ...
  record field chains, record array-field element extraction, and fixed-array literals whose leaf elements carry
  explicit types. Whole-value reassignment for those same mutable local aggregate subsets now reuses direct typed
  aggregate stores, and supported mutable-local plus pointer-backed aggregate
  ...
  `pointer.rows[index][inner] = value`. Broader aggregate construction/assignment remains future work.
