...
auto main() -> int {
    ...
    constexpr auto backend_examples = std::array<std::string_view, 89> {
    ...
    assert(
        computed_local_nested_same_owner_dynamic_array_example.ir_text.find(
            "define i32 @main()"
        ) != std::string::npos
    );
    assert(
    ...
    auto choice_distinct_switch = choice_dynamic_array_distinct_payload.ir_text.find(
    ...
    auto choice_distinct_parent_deallocate_after_switch = choice_dynamic_array_distinct_payload.ir_text.find(
        "call void @__orison_dynamic_array_deallocate(ptr "
        "%buffer.Ready.values.choice_dynamic_array_cleanup0.cleanup.data",
    ...
    auto choice_return_switch_payload = choice_dynamic_array_distinct_payload.ir_text.find(
        "define { ptr, i64, i64 } @return_switch_payload({ i32, { ptr, i64, i64 } } %buffer)"
    );
    auto choice_return_switch_payload_end = choice_dynamic_array_distinct_payload.ir_text.find(
        "define { i32, [24 x i8] } @make_buffer",
        choice_return_switch_payload
    );
    auto choice_return_switch_payload_phi = choice_dynamic_array_distinct_payload.ir_text.find(
        "phi { ptr, i64, i64 }",
        choice_return_switch_payload
    );
    auto choice_return_switch_payload_value_cleanup = choice_dynamic_array_distinct_payload.ir_text.find(
        "%values.dynamic_array_cleanup",
        choice_return_switch_payload
    );
    auto choice_return_switch_payload_parent_cleanup = choice_dynamic_array_distinct_payload.ir_text.find(
    ...
    assert(choice_distinct_switch != std::string::npos);
    ...
    assert(choice_return_switch_payload != std::string::npos);
    assert(choice_return_switch_payload_end != std::string::npos);
    assert(choice_return_switch_payload_phi != std::string::npos);
    assert(choice_return_switch_payload_value_cleanup == std::string::npos ||
           choice_return_switch_payload_end < choice_return_switch_payload_value_cleanup);
