...
void test_parse_success() {
    ...
    assert(result.module.functions.front().body_statements[0].kind == orison::syntax::StatementKind::if_statement);
    ...
    assert(result.module.functions.front().body_statements[0].nested_statements[0].expression.left->kind == orison::syntax::ExpressionKind::member_access);
    assert(result.module.functions.front().body_statements[0].nested_statements[0].expression.left->text == "read");
    assert(result.module.functions.front().body_statements[0].nested_statements[0].expression.left->left->text == "input");
    assert(result.module.functions.front().body_statements[0].nested_statements[0].expression.arguments.size() == 1);
