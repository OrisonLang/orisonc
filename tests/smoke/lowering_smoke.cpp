...
void test_derives_dynamic_array_element_cleanup_from_semantic_descriptor_origin() {
    ...
    auto sequence_report = orison::lowering::format_dynamic_array_cleanup_sequence_plan_report(
        blocked.dynamic_array_cleanup_sequence_plans
    );
    ...
    auto verification_report = orison::lowering::format_dynamic_array_cleanup_sequence_verification_report(
        blocked.dynamic_array_cleanup_sequence_verifications
    );
...
void test_binds_test_only_dynamic_array_parameter_descriptor_origin() {
    ...
    assert(
        computed_same_owner_parameter_for.ir_text.find(
            "define i32 @sum_words(i1 %flag, { ptr, i64, i64 } %items)"
        ) != std::string::npos
    );
    assert(computed_same_owner_parameter_for.ir_text.find("items.computed_for.0.condition:\n") != std::string::npos);
    assert(computed_same_owner_parameter_for.ir_text.find("items.computed_for.0.body:\n") != std::string::npos);
    assert(computed_same_owner_parameter_for.ir_text.find("items.computed_for.0.exit:\n") != std::string::npos);
    assert(
        computed_same_owner_parameter_for.ir_text.find("items.computed_for.0.cleanup.resume.call") ==
        std::string::npos
    );
    auto same_owner_first_deallocate =
        computed_same_owner_parameter_for.ir_text.find("call void @__orison_dynamic_array_deallocate");
    ...
    assert(computed_local_same_owner_element_address.source_type_name == "DynamicArray<UInt32>");
    assert(computed_local_same_owner_element_address.element_source_type_name == "UInt32");
    assert(computed_local_same_owner_element_address.element_llvm_type_name == "i32");
    assert(computed_local_same_owner_element_address.data_pointer_name == "%items.computed_for.data");
    assert(computed_local_same_owner_element_address.index_name == "%items.computed_for.index");
    assert(computed_local_same_owner_element_address.element_address_name == "%items.computed_for.element.addr");
    assert(computed_local_same_owner_element_address.rendered_ir.size() == 1);
    auto computed_local_same_owner_element_address_report =
        computed_local_same_owner_for.computed_dynamic_array_for_element_address_render_report();
    assert(computed_local_same_owner_element_address_report.size() == 1);
    assert(
    ...
    assert(computed_local_same_owner_loop_continue.source_type_name == "DynamicArray<UInt32>");
    assert(computed_local_same_owner_loop_continue.element_source_type_name == "UInt32");
    assert(computed_local_same_owner_loop_continue.continue_block_name == "items.computed_for.continue");
    assert(computed_local_same_owner_loop_continue.condition_block_name == "items.computed_for.condition");
    assert(computed_local_same_owner_loop_continue.index_name == "%items.computed_for.index");
    assert(computed_local_same_owner_loop_continue.next_index_name == "%items.computed_for.next.index");
    assert(computed_local_same_owner_loop_continue.rendered_ir.size() == 3);
    auto computed_local_same_owner_loop_continue_report =
        computed_local_same_owner_for.computed_dynamic_array_for_loop_continue_render_report();
    assert(computed_local_same_owner_loop_continue_report.size() == 1);
    assert(
    ...
    auto computed_local_nested_owner_mismatch_source =
        ...
        "        total = total + word\n"
        "    total\n";
    auto computed_local_nested_owner_mismatch_for = lower_source(
        path,
        computed_local_nested_owner_mismatch_source,
        orison::lowering::LlvmIrEmissionOptions {
            .enable_dynamic_array_construction_lowering = true,
            .enable_dynamic_array_for_lowering = true,
        }
    );
    assert(computed_local_nested_owner_mismatch_for.has_errors());
    assert(
        computed_local_nested_owner_mismatch_for.render(path.string()).find(
            "computed DynamicArray ownership plan ternary branch owner mismatch source DynamicArray<UInt32> "
            "element UInt32 owners items items other [ownership join blocked] [cleanup owner blocked] (metadata only)"
        ) != std::string::npos
    );
    ...
    auto computed_local_same_owner_after_while_source =
        "package demo.dynamicarray\n"
        "\n"
        "function sum_words(flag: Bool) -> UInt32\n"
        "    let items: DynamicArray<UInt32> = DynamicArray()\n"
        ...
        "        total = total + word\n"
        "    total\n";
    auto computed_local_same_owner_after_while = lower_source(
        path,
        computed_local_same_owner_after_while_source,
        orison::lowering::LlvmIrEmissionOptions {
            .enable_dynamic_array_construction_lowering = true,
            .enable_dynamic_array_for_lowering = true,
            .enable_dynamic_array_cleanup_emission = true,
    ...
    assert(computed_local_same_owner_after_while.ir_text.find("[cleanup calls disabled]") == std::string::npos);
    assert(computed_local_same_owner_after_while.ir_text.find("[cleanup calls enabled]") != std::string::npos);
    assert(
        computed_local_same_owner_after_while.ir_text.find(
            "call void @__orison_dynamic_array_deallocate(ptr %items.computed_for."
        ) != std::string::npos
    );
...
void test_emit_method_returned_view_for_iterable_lowering() {
    auto path = std::filesystem::temp_directory_path() / "orison_lowering_method_returned_view_for_iterable.or";
    auto result = lower_source(
        path,
        "package demo.lowering\n"
        "\n"
    ...
    assert_ir_contains(result, "define i32 @sum(i32 %seed, { ptr, i64 } %values)");
    assert_ir_contains(
        result,
...
void test_reject_foreign_alias_colliding_with_generic_specialization_symbol() {
    auto path = std::filesystem::temp_directory_path() / "orison_lowering_foreign_generic_symbol_collision.or";
    auto result = lower_source(
        path,
        "package demo.symbols\n"
        "\n"
        "package foreign \"c\"\n"
        "    function host_id(value: UInt32) -> UInt32 as \"id__UInt32\"\n"
        "\n"
    ...
    assert(
        result.diagnostics.entries().front().message ==
        "LLVM symbol 'id__UInt32' for foreign declaration collides with generated generic function specialization"
    );
}

void test_reject_foreign_alias_colliding_with_method_symbol() {
    auto path = std::filesystem::temp_directory_path() / "orison_lowering_foreign_method_symbol_collision.or";
    auto result = lower_source(
        path,
        "package demo.symbols\n"
        "\n"
        "package foreign \"c\"\n"
        "    function host_read(box: UInt32) -> UInt32 as \"method.Box_UInt32_.read\"\n"
        "\n"
        "record Box<T>\n"
        "    value: T\n"
        "\n"
    ...
    assert(
        result.diagnostics.entries().front().message ==
        "LLVM symbol 'method.Box_UInt32_.read' for foreign declaration collides with generated method symbol"
    );
}
