...
void assert_cli_runtime_indexed_branch_computed_member_cleanup_readiness_fixture_ready(
    ...
    assert(output.find(
        ...
        "element Box moved Inner member-path item source-line 16 source-text "
        "var outer: Outer = Outer(items[choose_index(true)].item) proof ready target-metadata ready "
        "helper-drop-bindings ready cfg-slice ready module-mutation ready production-member-cleanup ready "
        "production-gate ready production-enabled true production ready blockers 0"
    ) != std::string::npos);
    assert(output.find(
...
void assert_cli_runtime_indexed_choice_payload_computed_member_cleanup_readiness_fixture_ready(
    ...
    assert(output.find(
        "runtime-index member cleanup production-readiness owner items index (index + zero) "
        "element Box moved Inner member-path item source-line 26 source-text "
        "var outer: Outer = Outer(items[index + zero].item) proof ready target-metadata ready "
        "helper-drop-bindings ready cfg-slice ready module-mutation ready production-member-cleanup ready "
        "production-gate ready production-enabled true production ready blockers 0"
    ) != std::string::npos);
    assert(output.find(
...
void assert_cli_runtime_indexed_single_member_cleanup_emit_llvm_fixture_success(
    ...
    assert(deallocate != std::string::npos);
    assert(zero_descriptor != std::string::npos);
    assert(index_expression < moved_member_load);
...
void assert_cli_runtime_indexed_choice_payload_nested_computed_member_cleanup_emit_llvm_fixture_success(
    ...
    auto zero_descriptor = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp2");
    assert(output.find("declare void @__orison_dynamic_array_bounds_failed()") != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_out_of_bounds_fixture_success(
    ...
    auto bounds_check = output.find(".in_bounds = icmp ult i64 %index", index_value);
    auto trap = output.find("call void @__orison_dynamic_array_bounds_failed()", bounds_check);
    auto descriptor_load = output.find("returned_aggregate_receiver_descriptor", trap);
