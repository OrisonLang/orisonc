...
void assert_cli_runtime_indexed_single_member_cleanup_emit_llvm_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
    auto output = read_command_output(command);
    auto index_expression = output.find("%tmp6 = add i64 %index, %zero");
...
void assert_cli_runtime_indexed_two_nested_member_cleanup_emit_llvm_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
    auto output = read_command_output(command);
    auto left_index_expression = output.find("%tmp24 = add i64 %left_index, %left_zero");
    auto left_moved_member_load = output.find("%tmp28 = load %record.Inner, ptr %tmp27");
    auto left_cleanup_branch = output.find("br label %left_items.member_cleanup.entry");
    auto left_skip_moved = output.find(
        "%left_items.member_cleanup.is_moved = icmp eq i64 %left_items.member_cleanup.index, %tmp24"
    );
    auto left_member_helper = output.find(
    ...
    auto right_index_expression = output.find("%tmp54 = add i64 %right_index, %right_zero");
    auto right_moved_member_load = output.find("%tmp58 = load %record.Inner, ptr %tmp57");
    auto right_cleanup_entry = output.find("right_items.member_cleanup.entry:");
    auto right_skip_moved = output.find(
        "%right_items.member_cleanup.is_moved = icmp eq i64 %right_items.member_cleanup.index, %tmp54"
    );
    auto right_member_helper = output.find(
...
void assert_cli_emit_llvm_dynamic_array_receiver_direct_owned_method_chain_count_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
...
void assert_cli_emit_llvm_dynamic_array_receiver_direct_owned_ternary_method_chain_count_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
    ...
    assert(output.find("define { ptr, i64, i64 } @method.DynamicArray_Payload_.forward__Payload(ptr %this)") !=
        std::string::npos);
    assert(output.find("%tmp2 = phi { ptr, i64, i64 }") != std::string::npos);
    assert(output.find("store { ptr, i64, i64 } %tmp2, ptr %dynamic_array_receiver_tmp3.addr") !=
        std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_direct_owned_ternary_method_chain_append_statement_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_aggregate_field_method_chain_count_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
    ...
    assert(output.find("define { ptr, i64, i64 } @method.DynamicArray_Payload_.forward__Payload(ptr %this)") !=
        std::string::npos);
    assert(output.find("call %record.PayloadBox @make_box()") != std::string::npos);
    assert(output.find("getelementptr %record.PayloadBox") != std::string::npos);
    assert(output.find("call { ptr, i64, i64 } @method.DynamicArray_Payload_.forward__Payload(ptr %dynamic_array_receiver_tmp4.addr)") !=
        std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_aggregate_field_method_chain_append_statement_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_fixture_success(
    ...
    assert(output.find(".in_bounds = icmp ult i64 %index") != std::string::npos);
    assert(output.find("call void @__orison_dynamic_array_bounds_failed()") != std::string::npos);
    ...
    assert(output.find("call void @__orison_dynamic_array_deallocate(ptr %dynamic_array_receiver_tmp") !=
        std::string::npos);
    assert(output.find("call void @__orison_owned_cleanup.Bucket(ptr %dynamic_array_receiver_aggregate_tmp") !=
        std::string::npos);
    assert(output.find("call void @__orison_dynamic_array_deallocate(ptr %dynamic_array_receiver_aggregate_tmp") !=
        std::string::npos);
    assert(output.find("ret i32") != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_append_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path,
    bool expect_append
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
    auto output = read_command_output(command);
    auto selected_descriptor = output.find("returned_aggregate_receiver_descriptor");
    auto selected_zero = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp", selected_descriptor);
    auto receiver_cleanup = output.find("call void @__orison_owned_cleanup.Payload(ptr %dynamic_array_receiver_tmp");
    ...
    assert(output.find("call void @__orison_owned_cleanup.BoxedValues(ptr %Bucket.drop.box.addr)") !=
        std::string::npos);
    assert(output.find("named_dynamic_array_receiver_descriptor") == std::string::npos);
    if (expect_append) {
        assert(output.find("call void @method.DynamicArray_Payload_.append_value__Payload") != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_returned_dynamic_array_element_sibling_descriptor_assignment_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
    ...
    auto items_projection = output.find(".items.addr", root_storage);
    ...
    assert(second_bounds_check != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_out_of_bounds_success(
    ...
    auto output = read_command_output(command);
    auto index_value = output.find("%index = add i64 0, 2");
    auto bounds_check = output.find(".in_bounds = icmp ult i64 %index");
    auto trap = output.find("call void @__orison_dynamic_array_bounds_failed()");
    auto selected_descriptor = output.find("returned_aggregate_receiver_descriptor");
    auto selected_zero = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp");
    auto root_cleanup = output.find("call void @__orison_owned_cleanup.Bucket(ptr %dynamic_array_receiver_aggregate_tmp");
    assert(index_value != std::string::npos);
    assert(bounds_check != std::string::npos);
    assert(trap != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_out_of_bounds_fixture_success(
    ...
    auto bounds_check = output.find(".in_bounds = icmp ult i64 %index");
    auto trap = output.find("call void @__orison_dynamic_array_bounds_failed()");
    auto descriptor_load = output.find("returned_aggregate_receiver_descriptor");
    auto named_descriptor_load = output.find("named_dynamic_array_receiver_descriptor");
    auto source_slot_zero = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp");
    assert(index_value != std::string::npos);
    assert(bounds_check != std::string::npos);
    assert(trap != std::string::npos);
    assert(descriptor_load != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_out_of_bounds_fixture_success(
    ...
    auto bounds_check = output.find(".in_bounds = icmp ult i64 %index");
    auto trap = output.find("call void @__orison_dynamic_array_bounds_failed()");
    auto descriptor_load = output.find("named_dynamic_array_receiver_descriptor");
    auto source_slot_zero = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp");
    assert(index_value != std::string::npos);
    assert(bounds_check != std::string::npos);
    assert(trap != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_computed_out_of_bounds_fixture_success(
    ...
    auto one_value = output.find("%one = add i64 0, 1");
    auto computed_index = output.find(" = add i64 %index, %one");
    auto bounds_check = output.find(".in_bounds = icmp ult i64");
    auto trap = output.find("call void @__orison_dynamic_array_bounds_failed()");
    auto descriptor_load = output.find("named_dynamic_array_receiver_descriptor");
    auto source_slot_zero = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp");
    assert(index_value != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_nested_out_of_bounds_fixture_success(
    ...
    auto bounds_check = output.find(".in_bounds = icmp ult i64");
    auto trap = output.find("call void @__orison_dynamic_array_bounds_failed()");
    auto bucket_field = output.find("getelementptr %record.Bucket");
    auto boxed_field = output.find("getelementptr %record.BoxedValues");
    auto descriptor_load = output.find("named_dynamic_array_receiver_descriptor");
    auto source_slot_zero = output.find("store { ptr, i64, i64 } zeroinitializer, ptr %tmp");
    assert(index_value != std::string::npos);
    assert(one_value != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_nested_append_fixture_success(
    std::filesystem::path const& executable,
    std::filesystem::path const& path
) {
    auto command = executable.string() + " --emit-llvm " + path.string();
...
void assert_cli_emit_llvm_dynamic_array_receiver_multi_payload_choice_nested_count_fixture_success(
    ...
    assert(marker_subtract != std::string::npos);
    assert(output.find("%items.addr = alloca { ptr, i64, i64 }") != std::string::npos);
    assert(output.find("dynamic_array.receiver_element_path.in_bounds") != std::string::npos);
    assert(output.find("getelementptr %record.Bucket") != std::string::npos);
    assert(output.find("getelementptr %record.BoxedValues") != std::string::npos);
...
void assert_cli_emit_llvm_dynamic_array_receiver_multi_variant_choice_nested_count_fixture_success(
    ...
    auto primary_case = output.find("switch.case.0.0");
    auto fallback_case = output.find("switch.case.0.1");
    auto payload_extract = output.find("extractvalue { i32, [32 x i8] } %packet, 1");
    auto payload_spill = output.find("alloca [32 x i8]");
    auto typed_payload_load = output.find("load { { ptr, i64, i64 }, i32 }");
    auto items_extract = output.find("extractvalue { { ptr, i64, i64 }, i32 }");
    auto marker_extract = output.find("extractvalue { { ptr, i64, i64 }, i32 }", items_extract + 1);
    auto count_call = output.find("call i64 @method.DynamicArray_Payload_.count__Payload");
    auto fallback_payload = output.find("load i32");
    auto merge_phi = output.find("phi i32");
...
void assert_cli_emit_llvm_dynamic_array_receiver_multi_payload_choice_nested_append_fixture_success(
    ...
    auto marker_extract = output.find("extractvalue { { ptr, i64, i64 }, i32 }", items_extract + 1);
    auto marker_compare = output.find("icmp eq i32", marker_extract);
    assert(payload_extract != std::string::npos);
    assert(items_extract != std::string::npos);
    assert(marker_extract != std::string::npos);
    assert(marker_compare != std::string::npos);
    assert(output.find("%items.addr = alloca { ptr, i64, i64 }") != std::string::npos);
    assert(output.find("dynamic_array.receiver_element_path.in_bounds") != std::string::npos);
    assert(output.find("getelementptr %record.Bucket") != std::string::npos);
    assert(output.find("getelementptr %record.BoxedValues") != std::string::npos);
...
void assert_cli_emit_llvm_choice_constructor_indexed_member_path_sibling_move_fixture_success(
    ...
    auto sibling_spare_cleanup =
        output.find("%sibling.Some.item.spare.choice_dynamic_array_cleanup");
    assert(main_start != std::string::npos);
    assert(selected_values_cleanup != std::string::npos);
    assert(selected_spare_cleanup != std::string::npos);
...
auto main(int argc, char** argv) -> int {
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_count_run.or",
        smoke_temp_root / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_count"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_count_run.or"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_out_of_bounds_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_count_out_of_bounds.or"
    );
    assert_cli_run_existing_fixture_failure(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_count_out_of_bounds.or"
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_append_statement_run.or",
        smoke_temp_root / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_append_statement"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_append_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_field_method_chain_append_statement_run.or"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_out_of_bounds_fixture_success(
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_field_method_chain_count"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_field_method_chain_count_run.or",
        false
    );
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_field_method_chain_append_statement"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_returned_dynamic_array_element_sibling_descriptor_field_method_chain_append_statement_run.or",
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_returned_dynamic_array_element_sibling_descriptor_field_index_assignment"
    );
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_named_dynamic_array_element_field_method_chain_count"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_named_dynamic_array_element_field_method_chain_count_run.or"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_out_of_bounds_fixture_success(
        executable,
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_named_dynamic_array_element_computed_index_field_method_chain_count"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_named_dynamic_array_element_computed_index_field_method_chain_count_run.or"
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_named_dynamic_array_element_computed_index_field_method_chain_append_statement"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_append_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_named_dynamic_array_element_computed_index_field_method_chain_append_statement_run.or"
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_named_dynamic_array_element_computed_index_nested_field_method_chain_append_statement"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_nested_append_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_named_dynamic_array_element_computed_index_nested_field_method_chain_append_statement_run.or"
    );
    assert_cli_emit_llvm_receiver_method_chain_reuse_failure(
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_multi_payload_choice_computed_index_nested_field_method_chain_count"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_multi_payload_choice_nested_count_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_multi_payload_choice_computed_index_nested_field_method_chain_count_run.or"
    );
    assert_cli_emit_llvm_receiver_method_chain_reuse_failure(
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        ...
        smoke_temp_root / "dynamic_array_receiver_multi_payload_choice_computed_index_nested_field_method_chain_append_statement"
    );
    assert_cli_emit_llvm_dynamic_array_receiver_multi_payload_choice_nested_append_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_multi_payload_choice_computed_index_nested_field_method_chain_append_statement_run.or"
    );
    assert_cli_emit_llvm_receiver_method_chain_reuse_failure(
    ...
    assert_cli_emit_llvm_dynamic_array_receiver_named_dynamic_array_element_nested_append_out_of_bounds_fixture_success(
        executable,
        fixtures / "dynamic_array_receiver_multi_payload_choice_computed_index_nested_field_method_chain_append_statement_out_of_bounds.or"
    );
    assert_cli_run_existing_fixture_failure(
        executable,
        fixtures / "dynamic_array_receiver_multi_payload_choice_computed_index_nested_field_method_chain_append_statement_out_of_bounds.or"
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
        fixtures / "dynamic_array_forwarded_returned_nested_aggregate_field_sibling_cleanup_run.or",
        smoke_temp_root / "dynamic_array_forwarded_returned_nested_aggregate_field_sibling_cleanup"
    );
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
    ...
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
        fixtures / "dynamic_array_returned_nested_aggregate_field_sibling_after_primary_cleanup_run.or",
        smoke_temp_root / "dynamic_array_returned_nested_aggregate_field_sibling_after_primary_cleanup"
    );
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
        fixtures / "dynamic_array_returned_nested_aggregate_field_sibling_after_primary_final_switch_run.or",
        smoke_temp_root / "dynamic_array_returned_nested_aggregate_field_sibling_after_primary_final_switch"
    );
    assert_cli_dynamic_array_owned_result_fixture_full_production_success(
        executable,
    ...
    assert_cli_runtime_indexed_single_member_cleanup_emit_llvm_fixture_success(
        executable,
    ...
    assert_cli_runtime_indexed_two_nested_member_cleanup_emit_llvm_fixture_success(
        executable,
        fixtures / "runtime_indexed_dynamic_array_constructor_two_computed_nested_member_sibling_transfers.or"
    );
