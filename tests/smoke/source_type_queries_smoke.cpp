...
auto harmless_int64_marker_statement() -> orison::syntax::StatementSyntax {
    auto statement = orison::syntax::StatementSyntax {};
    ...
    statement.expression = integer_literal("1");
    return statement;
}

auto dynamic_array_uint32_parameter(std::string parameter_name = "items") -> orison::syntax::ParameterSyntax {
...
auto payload_box_parameter(std::string parameter_name = "box") -> orison::syntax::ParameterSyntax {
    return orison::syntax::ParameterSyntax {
        .name = std::move(parameter_name),
        .type = orison::syntax::TypeSyntax {.name = "PayloadBox"},
    };
}

auto outer_payload_box_parameter(std::string parameter_name = "box") -> orison::syntax::ParameterSyntax {
    return orison::syntax::ParameterSyntax {
        .name = std::move(parameter_name),
        .type = orison::syntax::TypeSyntax {.name = "OuterPayloadBox"},
    };
}
...
auto register_payload_box_forwarding_signature(
    ...
    for (auto index = std::size_t {0}; index < parameter_count; ++index) {
        parameter_types.push_back("%record.PayloadBox");
        parameter_source_type_names.push_back("PayloadBox");
    ...
    context.functions.emplace(std::move(function_name), orison::lowering::LoweredFunctionSignature {
        .return_type = "%record.PayloadBox",
        .source_return_type_name = "PayloadBox",
...
auto register_outer_payload_box_forwarding_signature(
    orison::lowering::LoweringContext& context,
    std::string function_name,
    std::size_t parameter_count = 1
) -> void {
    ...
    parameter_signedness.reserve(parameter_count);
    for (auto index = std::size_t {0}; index < parameter_count; ++index) {
        parameter_types.push_back("%record.OuterPayloadBox");
        parameter_source_type_names.push_back("OuterPayloadBox");
        parameter_signedness.push_back(orison::lowering::IntegerSignedness::not_integer);
    }
    context.functions.emplace(std::move(function_name), orison::lowering::LoweredFunctionSignature {
...
auto register_dynamic_array_forwarding_signature(
    ...
    parameter_signedness.reserve(parameter_count);
    for (auto index = std::size_t {0}; index < parameter_count; ++index) {
        parameter_types.push_back(std::string {orison::lowering::dynamic_array_descriptor_llvm_type()});
        parameter_source_type_names.push_back("DynamicArray<UInt32>");
        parameter_signedness.push_back(orison::lowering::IntegerSignedness::not_integer);
    }
    context.functions.emplace(std::move(function_name), orison::lowering::LoweredFunctionSignature {
...
auto direct_payload_box_forwarding_function(std::string function_name) -> orison::syntax::FunctionSyntax {
    auto function = orison::syntax::FunctionSyntax {};
    function.name = std::move(function_name);
    function.parameters.push_back(payload_box_parameter());
    function.body_statements.push_back(expression_statement(name("box")));
    return function;
}

auto direct_payload_box_forwarding_function_with_harmless_local(std::string function_name)
    -> orison::syntax::FunctionSyntax {
    auto function = orison::syntax::FunctionSyntax {};
    function.name = std::move(function_name);
    function.parameters.push_back(payload_box_parameter());
...
auto direct_outer_payload_box_forwarding_function(std::string function_name) -> orison::syntax::FunctionSyntax {
    auto function = orison::syntax::FunctionSyntax {};
    function.name = std::move(function_name);
...
auto direct_outer_payload_box_forwarding_function_with_harmless_local(std::string function_name)
    -> orison::syntax::FunctionSyntax {
    auto function = orison::syntax::FunctionSyntax {};
    function.name = std::move(function_name);
    function.parameters.push_back(outer_payload_box_parameter());
...
auto direct_holder_forwarding_function_with_harmless_local(std::string function_name)
    -> orison::syntax::FunctionSyntax {
    auto function = orison::syntax::FunctionSyntax {};
    function.name = std::move(function_name);
    function.parameters.push_back(holder_parameter());
...
auto dynamic_array_let_statement(std::string name, orison::syntax::ExpressionSyntax expression)
    -> orison::syntax::StatementSyntax {
    auto statement = orison::syntax::StatementSyntax {};
    statement.kind = orison::syntax::StatementKind::let_binding;
    statement.name = std::move(name);
...
auto assignment_statement(orison::syntax::ExpressionSyntax target, orison::syntax::ExpressionSyntax expression)
    -> orison::syntax::StatementSyntax {
    auto statement = orison::syntax::StatementSyntax {};
    statement.kind = orison::syntax::StatementKind::assignment_statement;
    statement.assignment_target = std::move(target);
    statement.expression = std::move(expression);
...
auto three_statement_block(
    orison::syntax::StatementSyntax first,
    orison::syntax::StatementSyntax second,
    orison::syntax::StatementSyntax third
) -> std::vector<orison::syntax::StatementSyntax> {
    auto statements = std::vector<orison::syntax::StatementSyntax> {};
    statements.push_back(std::move(first));
    statements.push_back(std::move(second));
    statements.push_back(std::move(third));
    return statements;
}

auto four_statement_block(
    orison::syntax::StatementSyntax first,
    orison::syntax::StatementSyntax second,
    orison::syntax::StatementSyntax third,
    orison::syntax::StatementSyntax fourth
) -> std::vector<orison::syntax::StatementSyntax> {
    auto statements = std::vector<orison::syntax::StatementSyntax> {};
    statements.push_back(std::move(first));
    statements.push_back(std::move(second));
    statements.push_back(std::move(third));
    statements.push_back(std::move(fourth));
    return statements;
}
...
int main() {
    ...
    register_payload_box_forwarding_signature(context, "forward_payload_box");
    register_payload_box_forwarding_signature(context, "forward_payload_box_extra");
    register_payload_box_forwarding_signature(context, "forward_payload_box_with_harmless_local");
    register_payload_box_forwarding_signature(context, "forward_payload_box_mismatch", 2);
    ...
    register_outer_payload_box_forwarding_signature(context, "forward_outer_payload_box_with_harmless_local");
    ...
    register_holder_forwarding_signature(context, "forward_holder_with_harmless_local");
    ...
    register_dynamic_array_forwarding_signature(context, "forward_final_if_alias");
    ...
    register_dynamic_array_forwarding_signature(context, "forward_final_if_alias_with_multi_harmless_locals");
    register_dynamic_array_forwarding_signature(context, "forward_final_switch_alias_with_multi_harmless_locals");
    ...
    context.records.emplace("PayloadBox", orison::lowering::LoweredRecordLayout {
        .name = "PayloadBox",
        .llvm_type_name = "%record.PayloadBox",
        .fields = {
            orison::lowering::LoweredRecordField {
                .name = "values",
                .source_type_name = "DynamicArray<UInt32>",
    ...
    context.records.emplace("OuterPayloadBox", orison::lowering::LoweredRecordLayout {
        .name = "OuterPayloadBox",
        .llvm_type_name = "%record.OuterPayloadBox",
        .fields = {
            orison::lowering::LoweredRecordField {
                .name = "inner",
                .source_type_name = "PayloadBox",
                .llvm_type = "%record.PayloadBox",
                .index = 0,
            },
        },
    });
    ...
    state.source_type_names["other_returned"] = "PayloadBox";
    ...
    state.addressable_bindings["returned"] = orison::lowering::AddressableBinding {
        .type = orison::lowering::LoweredType {
            .type = "%record.PayloadBox",
            .signedness = orison::lowering::IntegerSignedness::not_integer,
        },
        .storage = "%returned.addr",
    };
    state.addressable_bindings["returned_outer"] = orison::lowering::AddressableBinding {
        .type = orison::lowering::LoweredType {
            .type = "%record.OuterPayloadBox",
            .signedness = orison::lowering::IntegerSignedness::not_integer,
        },
    ...
    state.dynamic_array_local_cleanup_plans.push_back(orison::lowering::DynamicArrayDescriptorCleanupPlan {
        .owner_name = "returned.values",
        .source_type_name = "DynamicArray<UInt32>",
        .element_source_type_name = "UInt32",
        .element_llvm_type = "i32",
        .descriptor_storage_name = "%returned.values.addr0",
        .descriptor_storage_status =
            orison::lowering::DynamicArrayDescriptorStorageStatus::lowered_local_descriptor,
        .element_size_bytes = 4,
    });
    state.dynamic_array_local_cleanup_plans.push_back(orison::lowering::DynamicArrayDescriptorCleanupPlan {
        .owner_name = "other_returned.values",
        .source_type_name = "DynamicArray<UInt32>",
        .element_source_type_name = "UInt32",
        .element_llvm_type = "i32",
        .descriptor_storage_name = "%other_returned.values.addr0",
        .descriptor_storage_status =
            orison::lowering::DynamicArrayDescriptorStorageStatus::lowered_local_descriptor,
        .element_size_bytes = 4,
    });
    state.dynamic_array_local_cleanup_plans.push_back(orison::lowering::DynamicArrayDescriptorCleanupPlan {
        .owner_name = "returned_outer.inner.values",
        .source_type_name = "DynamicArray<UInt32>",
        .element_source_type_name = "UInt32",
        .element_llvm_type = "i32",
    ...
    state.dynamic_array_local_cleanup_plans.push_back(orison::lowering::DynamicArrayDescriptorCleanupPlan {
        .owner_name = "other_returned_outer.inner.values",
        .source_type_name = "DynamicArray<UInt32>",
        .element_source_type_name = "UInt32",
        .element_llvm_type = "i32",
        .descriptor_storage_name = "%other_returned_outer.inner.values.addr0",
        .descriptor_storage_status =
            orison::lowering::DynamicArrayDescriptorStorageStatus::lowered_local_descriptor,
        .element_size_bytes = 4,
    });
    ...
    auto forward_payload_box_function = direct_payload_box_forwarding_function("forward_payload_box");
    context.source_functions["forward_payload_box"] = &forward_payload_box_function;

    auto forward_payload_box_with_harmless_local_function =
        direct_payload_box_forwarding_function_with_harmless_local("forward_payload_box_with_harmless_local");
    context.source_functions["forward_payload_box_with_harmless_local"] =
        &forward_payload_box_with_harmless_local_function;
    ...
    auto forward_outer_payload_box_with_harmless_local_function =
        direct_outer_payload_box_forwarding_function_with_harmless_local(
            "forward_outer_payload_box_with_harmless_local"
        );
    context.source_functions["forward_outer_payload_box_with_harmless_local"] =
        &forward_outer_payload_box_with_harmless_local_function;
    ...
    auto forward_holder_with_harmless_local_function =
        direct_holder_forwarding_function_with_harmless_local("forward_holder_with_harmless_local");
    context.source_functions["forward_holder_with_harmless_local"] =
        &forward_holder_with_harmless_local_function;
    ...
    forward_payload_box_extra_function.body_statements.push_back(expression_statement(name("box")));
    forward_payload_box_extra_function.body_statements.push_back(expression_statement(name("box")));
    context.source_functions["forward_payload_box_extra"] = &forward_payload_box_extra_function;

    auto forward_payload_box_mismatch_function = orison::syntax::FunctionSyntax {};
    forward_payload_box_mismatch_function.name = "forward_payload_box_mismatch";
    forward_payload_box_mismatch_function.parameters.push_back(payload_box_parameter("left_box"));
    forward_payload_box_mismatch_function.parameters.push_back(payload_box_parameter("right_box"));
    forward_payload_box_mismatch_function.body_statements.push_back(if_statement(
        one_statement_block(expression_statement(name("left_box"))),
        one_statement_block(expression_statement(name("right_box")))
    ));
    context.source_functions["forward_payload_box_mismatch"] = &forward_payload_box_mismatch_function;

    auto forward_outer_payload_box_extra_function = orison::syntax::FunctionSyntax {};
    forward_outer_payload_box_extra_function.name = "forward_outer_payload_box_extra";
    forward_outer_payload_box_extra_function.parameters.push_back(outer_payload_box_parameter());
    forward_outer_payload_box_extra_function.body_statements.push_back(expression_statement(name("box")));
    forward_outer_payload_box_extra_function.body_statements.push_back(expression_statement(name("box")));
    context.source_functions["forward_outer_payload_box_extra"] = &forward_outer_payload_box_extra_function;

    auto forward_outer_payload_box_mismatch_function = orison::syntax::FunctionSyntax {};
    forward_outer_payload_box_mismatch_function.name = "forward_outer_payload_box_mismatch";
    forward_outer_payload_box_mismatch_function.parameters.push_back(outer_payload_box_parameter("left_box"));
    forward_outer_payload_box_mismatch_function.parameters.push_back(outer_payload_box_parameter("right_box"));
    forward_outer_payload_box_mismatch_function.body_statements.push_back(if_statement(
        one_statement_block(expression_statement(name("left_box"))),
        one_statement_block(expression_statement(name("right_box")))
    ));
    ...
    auto forward_final_if_function = orison::syntax::FunctionSyntax {};
    forward_final_if_function.name = "forward_final_if";
    forward_final_if_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_if_function.body_statements.push_back(if_statement(
    ...
    context.source_functions["forward_final_if"] = &forward_final_if_function;

    auto forward_final_switch_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_function.name = "forward_final_switch";
    forward_final_switch_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_switch_function.body_statements.push_back(switch_statement(
        one_statement_block(expression_statement(name("items"))),
        one_statement_block(expression_statement(call("forward_items", name("items"))))
    ));
    context.source_functions["forward_final_switch"] = &forward_final_switch_function;

    auto forward_final_if_alias_function = orison::syntax::FunctionSyntax {};
    forward_final_if_alias_function.name = "forward_final_if_alias";
    forward_final_if_alias_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_if_alias_function.body_statements.push_back(if_statement(
        two_statement_block(
    ...
    context.source_functions["forward_final_if_alias"] = &forward_final_if_alias_function;

    auto forward_final_switch_alias_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_alias_function.name = "forward_final_switch_alias";
    forward_final_switch_alias_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_switch_alias_function.body_statements.push_back(switch_statement(
        two_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            expression_statement(name("alias"))
        ),
    ...
    context.source_functions["forward_final_switch_alias"] = &forward_final_switch_alias_function;

    auto forward_final_if_alias_with_extra_function = orison::syntax::FunctionSyntax {};
    forward_final_if_alias_with_extra_function.name = "forward_final_if_alias_with_extra";
    forward_final_if_alias_with_extra_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_if_alias_with_extra_function.body_statements.push_back(if_statement(
        three_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            expression_statement(name("items")),
            expression_statement(name("alias"))
        ),
        two_statement_block(
            dynamic_array_let_statement("alias", name("items")),
    ...
    context.source_functions["forward_final_if_alias_with_extra"] = &forward_final_if_alias_with_extra_function;

    auto forward_final_switch_alias_with_extra_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_alias_with_extra_function.name = "forward_final_switch_alias_with_extra";
    forward_final_switch_alias_with_extra_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_switch_alias_with_extra_function.body_statements.push_back(switch_statement(
        three_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            expression_statement(name("items")),
            expression_statement(name("alias"))
    ...
    auto forward_final_if_alias_with_multi_harmless_locals_function = orison::syntax::FunctionSyntax {};
    forward_final_if_alias_with_multi_harmless_locals_function.name =
        "forward_final_if_alias_with_multi_harmless_locals";
    forward_final_if_alias_with_multi_harmless_locals_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_if_alias_with_multi_harmless_locals_function.body_statements.push_back(if_statement(
        four_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            int64_var_statement("first", integer_literal("1")),
            int64_var_statement("second", integer_literal("2")),
            expression_statement(name("alias"))
        ),
        three_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            int64_var_statement("fallback", integer_literal("3")),
            expression_statement(name("alias"))
        )
    ));
    context.source_functions["forward_final_if_alias_with_multi_harmless_locals"] =
        &forward_final_if_alias_with_multi_harmless_locals_function;

    auto forward_final_switch_alias_with_multi_harmless_locals_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_alias_with_multi_harmless_locals_function.name =
        "forward_final_switch_alias_with_multi_harmless_locals";
    forward_final_switch_alias_with_multi_harmless_locals_function.parameters.push_back(
        dynamic_array_uint32_parameter()
    );
    forward_final_switch_alias_with_multi_harmless_locals_function.body_statements.push_back(switch_statement(
        four_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            int64_var_statement("first", integer_literal("1")),
            int64_var_statement("second", integer_literal("2")),
    ...
    context.source_functions["forward_final_switch_alias_with_multi_harmless_locals"] =
        &forward_final_switch_alias_with_multi_harmless_locals_function;

    auto forward_final_if_alias_with_owner_in_ternary_alternate_function = orison::syntax::FunctionSyntax {};
    forward_final_if_alias_with_owner_in_ternary_alternate_function.name =
        "forward_final_if_alias_with_owner_in_ternary_alternate";
    forward_final_if_alias_with_owner_in_ternary_alternate_function.parameters.push_back(
        dynamic_array_uint32_parameter()
    );
    forward_final_if_alias_with_owner_in_ternary_alternate_function.body_statements.push_back(if_statement(
        three_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            int64_var_statement(
                "marker",
    ...
    context.source_functions["forward_final_if_alias_with_owner_in_ternary_alternate"] =
        &forward_final_if_alias_with_owner_in_ternary_alternate_function;

    auto forward_final_switch_alias_with_owner_in_ternary_alternate_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_alias_with_owner_in_ternary_alternate_function.name =
        "forward_final_switch_alias_with_owner_in_ternary_alternate";
    forward_final_switch_alias_with_owner_in_ternary_alternate_function.parameters.push_back(
        dynamic_array_uint32_parameter()
    );
    forward_final_switch_alias_with_owner_in_ternary_alternate_function.body_statements.push_back(switch_statement(
        three_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            int64_var_statement(
                "marker",
    ...
    forward_final_if_alias_reassigned_function.body_statements.push_back(if_statement(
        three_statement_block(
            dynamic_array_var_statement("alias", name("items")),
    ...
    context.source_functions["forward_final_if_alias_reassigned"] = &forward_final_if_alias_reassigned_function;

    auto forward_final_switch_alias_reassigned_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_alias_reassigned_function.name = "forward_final_switch_alias_reassigned";
    forward_final_switch_alias_reassigned_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_switch_alias_reassigned_function.body_statements.push_back(switch_statement(
        three_statement_block(
            dynamic_array_var_statement("alias", name("items")),
            assignment_statement(name("alias"), name("items")),
            expression_statement(name("alias"))
    ...
    auto forward_final_if_switch_alias_function = orison::syntax::FunctionSyntax {};
    forward_final_if_switch_alias_function.name = "forward_final_if_switch_alias";
    forward_final_if_switch_alias_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_if_switch_alias_function.body_statements.push_back(if_statement(
        two_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            expression_statement(name("alias"))
        ),
    ...
    forward_final_if_mismatch_function.parameters.push_back(dynamic_array_uint32_parameter("right"));
    forward_final_if_mismatch_function.body_statements.push_back(if_statement(
        one_statement_block(expression_statement(name("left"))),
        one_statement_block(expression_statement(call("forward_items", name("right"))))
    ));
    context.source_functions["forward_final_if_mismatch"] = &forward_final_if_mismatch_function;

    auto forward_final_switch_mismatch_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_mismatch_function.name = "forward_final_switch_mismatch";
    forward_final_switch_mismatch_function.parameters.push_back(dynamic_array_uint32_parameter("left"));
    forward_final_switch_mismatch_function.parameters.push_back(dynamic_array_uint32_parameter("right"));
    forward_final_switch_mismatch_function.body_statements.push_back(switch_statement(
        one_statement_block(expression_statement(name("left"))),
        one_statement_block(expression_statement(call("forward_items", name("right"))))
    ));
    ...
    auto forward_final_switch_if_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_if_function.name = "forward_final_switch_if";
    forward_final_switch_if_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_switch_if_function.body_statements.push_back(switch_statement(
        one_statement_block(expression_statement(name("items"))),
    ...
    context.source_functions["forward_final_switch_if"] = &forward_final_switch_if_function;

    auto forward_final_switch_if_alias_function = orison::syntax::FunctionSyntax {};
    forward_final_switch_if_alias_function.name = "forward_final_switch_if_alias";
    forward_final_switch_if_alias_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_final_switch_if_alias_function.body_statements.push_back(switch_statement(
        two_statement_block(
            dynamic_array_let_statement("alias", name("items")),
            expression_statement(name("alias"))
        ),
    ...
    forward_final_switch_if_mismatch_function.parameters.push_back(dynamic_array_uint32_parameter("left"));
    forward_final_switch_if_mismatch_function.parameters.push_back(dynamic_array_uint32_parameter("right"));
    forward_final_switch_if_mismatch_function.body_statements.push_back(switch_statement(
        one_statement_block(expression_statement(name("left"))),
        one_statement_block(if_statement(
            one_statement_block(expression_statement(call("forward_items", name("right")))),
            one_statement_block(expression_statement(name("left")))
    ...
    context.source_functions["forward_final_switch_if_mismatch"] = &forward_final_switch_if_mismatch_function;
    ...
    forward_alias_with_extra_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_alias_with_extra_function.body_statements.push_back(dynamic_array_let_statement("alias", name("items")));
    ...
    auto forward_alias_with_multi_harmless_locals_function = orison::syntax::FunctionSyntax {};
    forward_alias_with_multi_harmless_locals_function.name = "forward_alias_with_multi_harmless_locals";
    forward_alias_with_multi_harmless_locals_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_alias_with_multi_harmless_locals_function.body_statements.push_back(
        dynamic_array_var_statement("alias", name("items"))
    );
    forward_alias_with_multi_harmless_locals_function.body_statements.push_back(
        int64_var_statement("first", integer_literal("1"))
    ...
    forward_alias_with_multi_harmless_locals_function.body_statements.push_back(expression_statement(name("alias")));
    context.source_functions["forward_alias_with_multi_harmless_locals"] =
        &forward_alias_with_multi_harmless_locals_function;

    auto forward_alias_with_owner_in_ternary_alternate_function = orison::syntax::FunctionSyntax {};
    forward_alias_with_owner_in_ternary_alternate_function.name =
        "forward_alias_with_owner_in_ternary_alternate";
    forward_alias_with_owner_in_ternary_alternate_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_alias_with_owner_in_ternary_alternate_function.body_statements.push_back(
        dynamic_array_var_statement("alias", name("items"))
    );
    forward_alias_with_owner_in_ternary_alternate_function.body_statements.push_back(int64_var_statement(
        "marker",
        ternary(name("flag"), integer_literal("1"), index(name("items"), integer_literal("0")))
    ));
    forward_alias_with_owner_in_ternary_alternate_function.body_statements.push_back(
    ...
    context.source_functions["forward_alias_with_owner_in_ternary_alternate"] =
        &forward_alias_with_owner_in_ternary_alternate_function;
    ...
    forward_alias_reassigned_function.parameters.push_back(dynamic_array_uint32_parameter());
    forward_alias_reassigned_function.body_statements.push_back(dynamic_array_var_statement("alias", name("items")));
    forward_alias_reassigned_function.body_statements.push_back(assignment_statement(name("alias"), name("items")));
    forward_alias_reassigned_function.body_statements.push_back(expression_statement(name("alias")));
    context.source_functions["forward_alias_reassigned"] = &forward_alias_reassigned_function;
    ...
    auto forwarded_parameter_computed_handoff_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                name("items"),
                call("forward_items", name("items"))
            ),
            context,
            state
        );
    assert(
        forwarded_parameter_computed_handoff_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(forwarded_parameter_computed_handoff_plan.source_owner_name == "items");
    assert(forwarded_parameter_computed_handoff_plan.handoff_owner_name == "items");
    ...
    auto final_if_alias_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_alias", name("items")),
                call("forward_final_if_alias", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_alias_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_if_alias_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_if_alias_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_if_alias_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_if_alias_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_if_alias_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_switch_alias_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_alias", name("items")),
                call("forward_final_switch_alias", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_alias_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_switch_alias_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_switch_alias_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_switch_alias_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_switch_alias_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_switch_alias_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_if_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if", name("items")),
                call("forward_final_if", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_if_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_if_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_if_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_if_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_if_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_switch_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch", name("items")),
                call("forward_final_switch", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_switch_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_switch_forwarded_parameter_plan.handoff_owner_name == "items");
    ...
    auto final_if_switch_alias_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_switch_alias", name("items")),
                call("forward_final_if_switch_alias", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_switch_alias_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_if_switch_alias_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_if_switch_alias_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_if_switch_alias_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_if_switch_alias_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_if_switch_alias_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_switch_if_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_if", name("items")),
                call("forward_final_switch_if", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_if_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_switch_if_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_switch_if_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_switch_if_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_switch_if_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_switch_if_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_switch_if_alias_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_if_alias", name("items")),
                call("forward_final_switch_if_alias", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_if_alias_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_switch_if_alias_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_switch_if_alias_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_switch_if_alias_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_switch_if_alias_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_switch_if_alias_forwarded_parameter_plan.cleanup_owner_proven);
    ...
    auto local_alias_with_multi_harmless_locals_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_alias_with_multi_harmless_locals", name("items")),
                call("forward_alias_with_multi_harmless_locals", name("items"))
            ),
            context,
            state
    ...
    assert(
        local_alias_with_multi_harmless_locals_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(local_alias_with_multi_harmless_locals_forwarded_parameter_plan.source_owner_name == "items");
    assert(local_alias_with_multi_harmless_locals_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(local_alias_with_multi_harmless_locals_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(local_alias_with_multi_harmless_locals_forwarded_parameter_plan.descriptor_storage_available);
    assert(local_alias_with_multi_harmless_locals_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_alias_with_multi_harmless_locals", name("items")),
                call("forward_final_if_alias_with_multi_harmless_locals", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan.descriptor_storage_name == "%items.addr");
    assert(final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_if_alias_with_multi_harmless_locals_forwarded_parameter_plan.cleanup_owner_proven);

    auto final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_alias_with_multi_harmless_locals", name("items")),
                call("forward_final_switch_alias_with_multi_harmless_locals", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan.source_owner_name == "items");
    assert(final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan.handoff_owner_name == "items");
    assert(
        final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan.descriptor_storage_name ==
        "%items.addr"
    );
    assert(final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan.descriptor_storage_available);
    assert(final_switch_alias_with_multi_harmless_locals_forwarded_parameter_plan.cleanup_owner_proven);

    auto local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_alias_with_owner_in_ternary_alternate", name("items")),
                call("forward_alias_with_owner_in_ternary_alternate", name("items"))
            ),
            context,
            state
    ...
    assert(
        local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    assert(!local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.descriptor_storage_available);
    assert(!local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!local_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.lowering_enabled);

    auto final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_alias_with_owner_in_ternary_alternate", name("items")),
                call("forward_final_if_alias_with_owner_in_ternary_alternate", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(
        final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan
            .ownership_plan.branch_owner_names.empty()
    );
    assert(!final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.descriptor_storage_available);
    assert(!final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!final_if_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.lowering_enabled);

    auto final_switch_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_alias_with_owner_in_ternary_alternate", name("items")),
                call("forward_final_switch_alias_with_owner_in_ternary_alternate", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        final_switch_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(
        final_switch_alias_with_owner_in_ternary_alternate_forwarded_parameter_plan
            .ownership_plan.branch_owner_names.empty()
    );
    ...
    auto final_if_forwarded_parameter_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_mismatch", name("left"), name("right")),
                call("forward_final_if_mismatch", name("left"), name("right"))
            ),
            context,
            state
    ...
    assert(
        final_if_forwarded_parameter_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(final_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names.size() == 4);
    assert(final_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[0] == "left");
    assert(final_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[1] == "right");
    assert(final_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[2] == "left");
    assert(final_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[3] == "right");
    assert(!final_if_forwarded_parameter_mismatch_plan.descriptor_storage_available);
    assert(!final_if_forwarded_parameter_mismatch_plan.cleanup_owner_proven);
    assert(!final_if_forwarded_parameter_mismatch_plan.lowering_enabled);

    auto final_switch_forwarded_parameter_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_mismatch", name("left"), name("right")),
                call("forward_final_switch_mismatch", name("left"), name("right"))
            ),
            context,
            state
    ...
    assert(
        final_switch_forwarded_parameter_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(final_switch_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names.size() == 4);
    assert(final_switch_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[0] == "left");
    assert(final_switch_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[1] == "right");
    assert(final_switch_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[2] == "left");
    assert(final_switch_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[3] == "right");
    ...
    auto final_switch_if_forwarded_parameter_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_if_mismatch", name("left"), name("right")),
                call("forward_final_switch_if_mismatch", name("left"), name("right"))
            ),
            context,
            state
    ...
    assert(
        final_switch_if_forwarded_parameter_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names.size() == 6);
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[0] == "left");
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[1] == "right");
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[2] == "left");
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[3] == "left");
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[4] == "right");
    assert(final_switch_if_forwarded_parameter_mismatch_plan.ownership_plan.branch_owner_names[5] == "left");
    assert(!final_switch_if_forwarded_parameter_mismatch_plan.descriptor_storage_available);
    assert(!final_switch_if_forwarded_parameter_mismatch_plan.cleanup_owner_proven);
    assert(!final_switch_if_forwarded_parameter_mismatch_plan.lowering_enabled);

    auto final_if_alias_extra_statement_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_alias_with_extra", name("items")),
                call("forward_final_if_alias_with_extra", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_alias_extra_statement_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        final_if_alias_extra_statement_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(final_if_alias_extra_statement_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    assert(!final_if_alias_extra_statement_forwarded_parameter_plan.descriptor_storage_available);
    assert(!final_if_alias_extra_statement_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!final_if_alias_extra_statement_forwarded_parameter_plan.lowering_enabled);

    auto final_switch_alias_extra_statement_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_alias_with_extra", name("items")),
                call("forward_final_switch_alias_with_extra", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_alias_extra_statement_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        final_switch_alias_extra_statement_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(final_switch_alias_extra_statement_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    assert(!final_switch_alias_extra_statement_forwarded_parameter_plan.descriptor_storage_available);
    assert(!final_switch_alias_extra_statement_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!final_switch_alias_extra_statement_forwarded_parameter_plan.lowering_enabled);

    auto final_if_alias_reassigned_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_if_alias_reassigned", name("items")),
                call("forward_final_if_alias_reassigned", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_if_alias_reassigned_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        final_if_alias_reassigned_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(final_if_alias_reassigned_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    assert(!final_if_alias_reassigned_forwarded_parameter_plan.descriptor_storage_available);
    assert(!final_if_alias_reassigned_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!final_if_alias_reassigned_forwarded_parameter_plan.lowering_enabled);

    auto final_switch_alias_reassigned_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_final_switch_alias_reassigned", name("items")),
                call("forward_final_switch_alias_reassigned", name("items"))
            ),
            context,
            state
    ...
    assert(
        final_switch_alias_reassigned_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        final_switch_alias_reassigned_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(final_switch_alias_reassigned_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    ...
    auto local_alias_extra_statement_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_alias_with_extra", name("items")),
                call("forward_alias_with_extra", name("items"))
            ),
            context,
            state
    ...
    assert(
        local_alias_extra_statement_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        local_alias_extra_statement_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(local_alias_extra_statement_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    assert(!local_alias_extra_statement_forwarded_parameter_plan.descriptor_storage_available);
    assert(!local_alias_extra_statement_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!local_alias_extra_statement_forwarded_parameter_plan.lowering_enabled);

    auto local_alias_reassigned_forwarded_parameter_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                call("forward_alias_reassigned", name("items")),
                call("forward_alias_reassigned", name("items"))
            ),
            context,
            state
    ...
    assert(
        local_alias_reassigned_forwarded_parameter_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        local_alias_reassigned_forwarded_parameter_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(local_alias_reassigned_forwarded_parameter_plan.ownership_plan.branch_owner_names.empty());
    assert(!local_alias_reassigned_forwarded_parameter_plan.descriptor_storage_available);
    assert(!local_alias_reassigned_forwarded_parameter_plan.cleanup_owner_proven);
    assert(!local_alias_reassigned_forwarded_parameter_plan.lowering_enabled);
    ...
    assert(
        aggregate_field_dynamic_array_plan.kind ==
        orison::lowering::DynamicArrayIterableDescriptorPlanKind::named_descriptor_owner
    );
    assert(aggregate_field_dynamic_array_plan.source_type_name == "DynamicArray<UInt32>");
    assert(aggregate_field_dynamic_array_plan.element_source_type_name == "UInt32");
    assert(aggregate_field_dynamic_array_plan.owner_name == "returned.values");
    assert(aggregate_field_dynamic_array_plan.descriptor_storage == "%returned.values.addr0");
    assert(aggregate_field_dynamic_array_plan.can_lower_now);
    assert(aggregate_field_dynamic_array_plan.cleanup_owner_proven);
    auto aggregate_field_computed_handoff_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(name("returned"), "values"),
                member(name("returned"), "values")
            ),
            context,
            state
    ...
    assert(
        aggregate_field_computed_handoff_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(aggregate_field_computed_handoff_plan.source_owner_name == "returned.values");
    assert(aggregate_field_computed_handoff_plan.handoff_owner_name == "returned.values");
    assert(aggregate_field_computed_handoff_plan.descriptor_storage_name == "%returned.values.addr0");
    assert(aggregate_field_computed_handoff_plan.descriptor_storage_available);
    assert(aggregate_field_computed_handoff_plan.cleanup_owner_proven);

    auto forwarded_aggregate_field_computed_handoff_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(call("forward_payload_box", name("returned")), "values"),
                member(call("forward_payload_box", name("returned")), "values")
            ),
            context,
            state
    ...
    assert(
        forwarded_aggregate_field_computed_handoff_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(forwarded_aggregate_field_computed_handoff_plan.source_owner_name == "returned.values");
    assert(forwarded_aggregate_field_computed_handoff_plan.handoff_owner_name == "returned.values");
    assert(forwarded_aggregate_field_computed_handoff_plan.descriptor_storage_name == "%returned.values.addr0");
    assert(forwarded_aggregate_field_computed_handoff_plan.descriptor_storage_available);
    assert(forwarded_aggregate_field_computed_handoff_plan.cleanup_owner_proven);

    auto forwarded_aggregate_field_harmless_local_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(call("forward_payload_box_with_harmless_local", name("returned")), "values"),
                member(call("forward_payload_box_with_harmless_local", name("returned")), "values")
            ),
            context,
            state
    ...
    assert(
        forwarded_aggregate_field_harmless_local_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(forwarded_aggregate_field_harmless_local_plan.source_owner_name == "returned.values");
    assert(forwarded_aggregate_field_harmless_local_plan.handoff_owner_name == "returned.values");
    assert(forwarded_aggregate_field_harmless_local_plan.descriptor_storage_name == "%returned.values.addr0");
    assert(forwarded_aggregate_field_harmless_local_plan.descriptor_storage_available);
    assert(forwarded_aggregate_field_harmless_local_plan.cleanup_owner_proven);

    auto forwarded_aggregate_field_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(call("forward_payload_box", name("returned")), "values"),
                member(call("forward_payload_box", name("other_returned")), "values")
            ),
            context,
            state
        );
    assert(
        forwarded_aggregate_field_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(forwarded_aggregate_field_mismatch_plan.ownership_plan.branch_owner_names.size() == 2);
    assert(forwarded_aggregate_field_mismatch_plan.ownership_plan.branch_owner_names[0] == "returned.values");
    assert(forwarded_aggregate_field_mismatch_plan.ownership_plan.branch_owner_names[1] == "other_returned.values");
    assert(!forwarded_aggregate_field_mismatch_plan.descriptor_storage_available);
    assert(!forwarded_aggregate_field_mismatch_plan.cleanup_owner_proven);
    assert(!forwarded_aggregate_field_mismatch_plan.lowering_enabled);

    auto forwarded_aggregate_field_extra_statement_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(call("forward_payload_box_extra", name("returned")), "values"),
                member(call("forward_payload_box_extra", name("returned")), "values")
            ),
            context,
            state
    ...
    assert(
        forwarded_aggregate_field_extra_statement_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        forwarded_aggregate_field_extra_statement_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(forwarded_aggregate_field_extra_statement_plan.ownership_plan.branch_owner_names.empty());
    assert(!forwarded_aggregate_field_extra_statement_plan.descriptor_storage_available);
    assert(!forwarded_aggregate_field_extra_statement_plan.cleanup_owner_proven);
    assert(!forwarded_aggregate_field_extra_statement_plan.lowering_enabled);

    auto forwarded_aggregate_field_helper_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(call("forward_payload_box_mismatch", name("returned"), name("other_returned")), "values"),
                member(call("forward_payload_box_mismatch", name("returned"), name("other_returned")), "values")
            ),
            context,
            state
    ...
    assert(
        forwarded_aggregate_field_helper_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(forwarded_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names.size() == 4);
    assert(forwarded_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[0] == "returned.values");
    assert(
        forwarded_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[1] ==
        "other_returned.values"
    );
    assert(forwarded_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[2] == "returned.values");
    assert(
        forwarded_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[3] ==
        "other_returned.values"
    );
    assert(!forwarded_aggregate_field_helper_mismatch_plan.descriptor_storage_available);
    assert(!forwarded_aggregate_field_helper_mismatch_plan.cleanup_owner_proven);
    assert(!forwarded_aggregate_field_helper_mismatch_plan.lowering_enabled);

    auto nested_aggregate_field_dynamic_array_plan =
        orison::lowering::plan_dynamic_array_iterable_descriptor(
            member(member(name("returned_outer"), "inner"), "values"),
            context,
            state
        );
    assert(
        nested_aggregate_field_dynamic_array_plan.kind ==
        orison::lowering::DynamicArrayIterableDescriptorPlanKind::named_descriptor_owner
    );
    assert(nested_aggregate_field_dynamic_array_plan.source_type_name == "DynamicArray<UInt32>");
    assert(nested_aggregate_field_dynamic_array_plan.element_source_type_name == "UInt32");
    assert(nested_aggregate_field_dynamic_array_plan.owner_name == "returned_outer.inner.values");
    assert(nested_aggregate_field_dynamic_array_plan.descriptor_storage == "%returned_outer.inner.values.addr0");
    assert(nested_aggregate_field_dynamic_array_plan.can_lower_now);
    assert(nested_aggregate_field_dynamic_array_plan.cleanup_owner_proven);
    auto nested_aggregate_field_computed_handoff_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(member(name("returned_outer"), "inner"), "values"),
    ...
    assert(
        nested_aggregate_field_computed_handoff_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(nested_aggregate_field_computed_handoff_plan.source_owner_name == "returned_outer.inner.values");
    assert(nested_aggregate_field_computed_handoff_plan.handoff_owner_name == "returned_outer.inner.values");
    assert(nested_aggregate_field_computed_handoff_plan.descriptor_storage_name ==
    ...
    auto forwarded_nested_aggregate_field_computed_handoff_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(member(call("forward_outer_payload_box", name("returned_outer")), "inner"), "values"),
                member(member(call("forward_outer_payload_box", name("returned_outer")), "inner"), "values")
            ),
            context,
            state
    ...
    assert(
        forwarded_nested_aggregate_field_computed_handoff_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(forwarded_nested_aggregate_field_computed_handoff_plan.source_owner_name ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_computed_handoff_plan.handoff_owner_name ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_computed_handoff_plan.descriptor_storage_name ==
        "%returned_outer.inner.values.addr0");
    assert(forwarded_nested_aggregate_field_computed_handoff_plan.descriptor_storage_available);
    assert(forwarded_nested_aggregate_field_computed_handoff_plan.cleanup_owner_proven);

    auto forwarded_nested_aggregate_field_harmless_local_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(
                    member(call("forward_outer_payload_box_with_harmless_local", name("returned_outer")), "inner"),
                    "values"
                ),
                member(
                    member(call("forward_outer_payload_box_with_harmless_local", name("returned_outer")), "inner"),
    ...
    assert(
        forwarded_nested_aggregate_field_harmless_local_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(forwarded_nested_aggregate_field_harmless_local_plan.source_owner_name ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_harmless_local_plan.handoff_owner_name ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_harmless_local_plan.descriptor_storage_name ==
        "%returned_outer.inner.values.addr0");
    assert(forwarded_nested_aggregate_field_harmless_local_plan.descriptor_storage_available);
    assert(forwarded_nested_aggregate_field_harmless_local_plan.cleanup_owner_proven);

    auto forwarded_nested_aggregate_field_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(member(call("forward_outer_payload_box", name("returned_outer")), "inner"), "values"),
                member(member(call("forward_outer_payload_box", name("other_returned_outer")), "inner"), "values")
            ),
            context,
            state
        );
    assert(
        forwarded_nested_aggregate_field_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(forwarded_nested_aggregate_field_mismatch_plan.ownership_plan.branch_owner_names.size() == 2);
    assert(forwarded_nested_aggregate_field_mismatch_plan.ownership_plan.branch_owner_names[0] ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_mismatch_plan.ownership_plan.branch_owner_names[1] ==
        "other_returned_outer.inner.values");
    assert(!forwarded_nested_aggregate_field_mismatch_plan.descriptor_storage_available);
    assert(!forwarded_nested_aggregate_field_mismatch_plan.cleanup_owner_proven);
    assert(!forwarded_nested_aggregate_field_mismatch_plan.lowering_enabled);

    auto forwarded_nested_aggregate_field_extra_statement_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(member(call("forward_outer_payload_box_extra", name("returned_outer")), "inner"), "values"),
                member(member(call("forward_outer_payload_box_extra", name("returned_outer")), "inner"), "values")
            ),
            context,
            state
    ...
    assert(
        forwarded_nested_aggregate_field_extra_statement_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::unsupported_computed_shape
    );
    assert(
        forwarded_nested_aggregate_field_extra_statement_plan.ownership_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableOwnershipPlanKind::unsupported_computed_shape
    );
    assert(forwarded_nested_aggregate_field_extra_statement_plan.ownership_plan.branch_owner_names.empty());
    assert(!forwarded_nested_aggregate_field_extra_statement_plan.descriptor_storage_available);
    assert(!forwarded_nested_aggregate_field_extra_statement_plan.cleanup_owner_proven);
    assert(!forwarded_nested_aggregate_field_extra_statement_plan.lowering_enabled);

    auto forwarded_nested_aggregate_field_helper_mismatch_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(
                    member(
                        call("forward_outer_payload_box_mismatch", name("returned_outer"), name("other_returned_outer")),
                        "inner"
                    ),
                    "values"
                ),
    ...
    assert(
        forwarded_nested_aggregate_field_helper_mismatch_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::ownership_join_blocked
    );
    assert(forwarded_nested_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names.size() == 4);
    assert(forwarded_nested_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[0] ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[1] ==
        "other_returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[2] ==
        "returned_outer.inner.values");
    assert(forwarded_nested_aggregate_field_helper_mismatch_plan.ownership_plan.branch_owner_names[3] ==
        "other_returned_outer.inner.values");
    ...
    auto forwarded_static_indexed_aggregate_field_harmless_local_plan =
        orison::lowering::plan_computed_dynamic_array_iterable_descriptor_handoff(
            ternary(
                name("flag"),
                member(index(member(call("forward_holder_with_harmless_local", name("holder")), "buckets")), "items"),
                member(
                    index(
                        member(call("forward_holder_with_harmless_local", name("holder")), "buckets"),
                        cast(integer_literal("0"), "UInt64")
                    ),
                    "items"
                )
    ...
    assert(
        forwarded_static_indexed_aggregate_field_harmless_local_plan.kind ==
        orison::lowering::ComputedDynamicArrayIterableDescriptorHandoffPlanKind::
            single_cleanup_owner_handoff_planned
    );
    assert(forwarded_static_indexed_aggregate_field_harmless_local_plan.source_owner_name ==
        "holder.buckets.element0.items");
    assert(forwarded_static_indexed_aggregate_field_harmless_local_plan.handoff_owner_name ==
        "holder.buckets.element0.items");
    assert(forwarded_static_indexed_aggregate_field_harmless_local_plan.descriptor_storage_name ==
        "%holder.buckets.element0.items.addr0");
    assert(forwarded_static_indexed_aggregate_field_harmless_local_plan.descriptor_storage_available);
    assert(forwarded_static_indexed_aggregate_field_harmless_local_plan.cleanup_owner_proven);
