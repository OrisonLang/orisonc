...
void BranchBindingScope::commit_ownership_transfers(
    OwnershipTransferState transfers
) {
    saved_ownership_transfers_ = std::move(transfers);
    state_.ownership_transfers = saved_ownership_transfers_;
}

auto BranchBindingScope::saved_ownership_transfers() const -> OwnershipTransferState const& {
    return saved_ownership_transfers_;
}
