...
auto has_authorized_owned_cleanup_definition(
...
auto has_runtime_indexed_cleanup_owned_cleanup_definition(
    semantics::OwnedCleanupImplementation const& implementation,
...
auto owned_cleanup_element_symbol(
...
auto emit_record_owned_cleanup_body(
    std::string_view source_type_name,
    LoweringContext const& context,
    std::vector<semantics::OwnedCleanupImplementation> const& implementations
) -> std::string {
    ...
    for (auto const& field : layout->second.fields) {
        ...
        output << emit_dynamic_array_owned_cleanup_field_sequence(
            *plan,
            field_pointer_name,
            prefix,
            owned_cleanup_element_symbol(plan->element_source_type_name, implementations)
        );
        continue;
    }
    ...
    for (auto const& field : layout->second.fields) {
        ...
        auto drop_symbol = owned_cleanup_element_symbol(field.source_type_name, implementations);
        ...
        if (dynamic_array_element_plan.has_value()) {
            output << emit_fixed_array_dynamic_array_owned_cleanup_field_sequence(
                ...
                *dynamic_array_element_plan,
                cleanup_name,
                *length_value,
                owned_cleanup_element_symbol(dynamic_array_element_plan->element_source_type_name, implementations)
            );
            continue;
        }

        auto element_drop_symbol = owned_cleanup_element_symbol(*element_source_type_name, implementations);
...
auto collect_owned_cleanup_definition_symbols(
    syntax::ModuleSyntax const& module,
    LoweringContext const& context,
    std::vector<semantics::OwnedCleanupLoweringAuthorization> const& authorizations,
    LlvmIrEmissionOptions const& options
...
void add_compiler_intrinsic_cleanup_dependency(
    ...
    syntax::ModuleSyntax const& module,
    LoweringContext const& context,
    std::vector<semantics::OwnedCleanupImplementation>& implementations,
    std::vector<std::string>& active_source_types
);

auto emit_owned_cleanup_definitions(
    syntax::ModuleSyntax const& module,
    LoweringContext const& context,
    std::vector<semantics::OwnedCleanupLoweringAuthorization> const& authorizations,
    ...
    auto emitted_symbols = collect_owned_cleanup_definition_symbols(module, context, authorizations, options);
    for (auto const& declaration : owned_cleanup_declarations) {
    ...
    for (auto const& symbol : emitted_symbols) {
        ...
        output << "entry:\n";
        output << emit_record_owned_cleanup_body(
            implementation->source_type_name,
            context,
            implementations
        );
...
auto owned_cleanup_implementation_for_type(
    std::string_view source_type_name,
    std::vector<semantics::OwnedCleanupImplementation> const& implementations
...
void add_owned_cleanup_definition_symbol_with_dependencies(
    std::string_view source_type_name,
    LoweringContext const& context,
    std::vector<semantics::OwnedCleanupImplementation> const& implementations,
    ...
    auto implementation = owned_cleanup_implementation_for_type(source_type_name, implementations);
    ...
    if (layout != context.records.end()) {
        for (auto const& field : layout->second.fields) {
            if (auto element_source_type = dynamic_array_element_source_type_name(field.source_type_name)) {
                add_owned_cleanup_definition_symbol_with_dependencies(
                    *element_source_type,
                    context,
                    implementations,
                    symbols,
...
auto collect_direct_owned_cleanup_definition_types(
    std::vector<semantics::OwnedCleanupImplementation> const& implementations,
    std::vector<semantics::OwnedCleanupLoweringAuthorization> const& authorizations,
    LlvmIrEmissionOptions const& options
) -> std::vector<std::string> {
    ...
    for (auto const& implementation : implementations) {
        if (!has_authorized_owned_cleanup_definition(implementation, authorizations) &&
            !has_runtime_indexed_cleanup_owned_cleanup_definition(implementation, options)) {
            continue;
        }
...
auto collect_owned_cleanup_definition_symbols(
    ...
    for (auto const& source_type : collect_direct_owned_cleanup_definition_types(
             implementations,
             authorizations,
             options
         )) {
...
auto member_cleanup_path_text(std::vector<std::string> const& path) -> std::string {
    if (path.empty()) {
        return "none";
    }
...
auto member_cleanup_site_fragment(Record const& record) -> std::string {
    auto text = std::ostringstream {};
    text << "owner " << record.owner_name
         ...
         << " member-path " << member_cleanup_path_text(record.moved_member_path);
    return text.str();
}
...
auto remove_helper_drop_binding_blocker_if_ready(
    ...
    std::erase(record.blockers, "member-helper-drop-bindings");
    return record.blockers.size() != old_size;
}

template <typename Record>
void replace_runtime_indexed_member_cleanup_audit_line(
...
void refresh_runtime_indexed_member_cleanup_production_readiness_with_helper_bindings(
    ...
    for (auto& entry : readiness) {
        ...
        replace_runtime_indexed_member_cleanup_audit_line(
            ...
            entry,
            runtime_indexed_member_cleanup_production_readiness_report(entry)
        );
    }
}
...
void refresh_runtime_indexed_member_cleanup_mutation_readiness_with_helper_bindings(
    ...
    std::vector<RuntimeIndexedMemberCleanupMutationApplyAuthorization>& apply_authorizations,
    std::vector<RuntimeIndexedMemberCleanupMutationApplyPreview>& apply_previews,
    std::vector<RuntimeIndexedMemberCleanupMutationPostApplyVerification>& post_apply_verifications,
    ...
    for (auto& entry : readiness) {
        if (remove_helper_drop_binding_blocker_if_ready(entry, helper_owned_cleanup_bindings)) {
            ...
            replace_runtime_indexed_member_cleanup_audit_line(
                ...
                entry,
                runtime_indexed_member_cleanup_mutation_production_readiness_report(entry)
            );
        }
    }
...
auto validate_prelude_module_symbols(
    ...
    for (auto const& symbol_name : owned_cleanup_symbols) {
        register_unique_module_symbol(
            registry,
            emitted_symbols,
            symbol_name,
            "owned cleanup definition",
            1,
            diagnostics
        );
    }
...
void append_function_emission_reports(
    ...
    result.runtime_indexed_member_cleanup_mutation_operation_plans.insert(
        ...
        function_emission.runtime_indexed_member_cleanup_mutation_operation_plans.end()
    );
    result.runtime_indexed_member_cleanup_mutation_operation_validations.insert(
        result.runtime_indexed_member_cleanup_mutation_operation_validations.end(),
        function_emission.runtime_indexed_member_cleanup_mutation_operation_validations.begin(),
        function_emission.runtime_indexed_member_cleanup_mutation_operation_validations.end()
    );
    result.runtime_indexed_member_cleanup_mutation_conflict_detections.insert(
        result.runtime_indexed_member_cleanup_mutation_conflict_detections.end(),
        function_emission.runtime_indexed_member_cleanup_mutation_conflict_detections.begin(),
        function_emission.runtime_indexed_member_cleanup_mutation_conflict_detections.end()
    );
    result.runtime_indexed_member_cleanup_mutation_apply_authorizations.insert(
        result.runtime_indexed_member_cleanup_mutation_apply_authorizations.end(),
        function_emission.runtime_indexed_member_cleanup_mutation_apply_authorizations.begin(),
        function_emission.runtime_indexed_member_cleanup_mutation_apply_authorizations.end()
    );
    result.runtime_indexed_member_cleanup_mutation_apply_previews.insert(
        result.runtime_indexed_member_cleanup_mutation_apply_previews.end(),
        function_emission.runtime_indexed_member_cleanup_mutation_apply_previews.begin(),
        function_emission.runtime_indexed_member_cleanup_mutation_apply_previews.end()
    );
    result.runtime_indexed_member_cleanup_mutation_post_apply_verifications.insert(
        result.runtime_indexed_member_cleanup_mutation_post_apply_verifications.end(),
        function_emission.runtime_indexed_member_cleanup_mutation_post_apply_verifications.begin(),
        function_emission.runtime_indexed_member_cleanup_mutation_post_apply_verifications.end()
    );
    result.runtime_indexed_member_cleanup_mutation_promotion_summaries.insert(
        result.runtime_indexed_member_cleanup_mutation_promotion_summaries.end(),
        function_emission.runtime_indexed_member_cleanup_mutation_promotion_summaries.begin(),
        function_emission.runtime_indexed_member_cleanup_mutation_promotion_summaries.end()
    );
    ...
    for (auto plan : function_emission.runtime_indexed_cleanup_emission_plans) {
        ...
        if (plan.ir_plan.descriptor_owner_ready) {
            plan.function_predecessor_block_name = function_emission.final_block_name;
        } else if (plan.function_predecessor_block_name.empty()) {
            plan.function_predecessor_block_name = function_emission.final_block_name;
        }
...
auto emit_module(
    ...
    auto owned_cleanup_symbols =
        collect_owned_cleanup_definition_symbols(module, context, result.semantic_owned_cleanup_lowering_authorizations, options);
    auto direct_owned_cleanup_symbols = collect_direct_owned_cleanup_definition_symbols(module);
    auto function_options = options;
    function_options.owned_cleanup_definition_symbols = owned_cleanup_symbols;
    function_options.owned_cleanup_definition_symbols.insert(
        function_options.owned_cleanup_definition_symbols.end(),
        direct_owned_cleanup_symbols.begin(),
        direct_owned_cleanup_symbols.end()
    );
    ...
    output << emit_owned_cleanup_definitions(
        module,
        context,
        result.semantic_owned_cleanup_lowering_authorizations,
        result.owned_cleanup_declarations,
