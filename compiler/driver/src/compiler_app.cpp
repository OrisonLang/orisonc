...
auto runtime_indexed_member_cleanup_execution_summary_report_lines(
    ...
    report << "runtime-index member cleanup execution-summary"
           ...
           << " apply " << (apply_authorization != nullptr && apply_authorization->apply_authorized ? "authorized" : "blocked")
...
auto runtime_indexed_member_cleanup_typed_promotion_state(
    ...
    for (auto const& gate : result.runtime_indexed_member_cleanup_typed_promotion_gates) {
        ready = ready && gate.production_enabled;
    }
    for (auto const& readiness : result.runtime_indexed_member_cleanup_mutation_production_readiness) {
...
auto test_only_runtime_indexed_member_cleanup_run(std::filesystem::path const& source_path) -> CompileResult {
    ...
    auto summary_lines = runtime_indexed_member_cleanup_execution_summary_report_lines(result);
