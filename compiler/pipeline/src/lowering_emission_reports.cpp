...
auto runtime_indexed_member_cleanup_helper_owned_cleanup_bindings_line(std::string const& line) -> bool {
    auto constexpr prefix = std::string_view {"runtime-index member cleanup helper-drop-bindings"};
    return line.starts_with(prefix);
}

auto runtime_indexed_member_cleanup_helper_body_line(std::string const& line) -> bool {
    auto constexpr prefix = std::string_view {"runtime-index member cleanup helper-body"};
    return line.starts_with(prefix);
}
...
auto runtime_indexed_member_cleanup_promotion_seam_line(std::string const& line) -> bool {
    auto constexpr prefix = std::string_view {"runtime-index member cleanup promotion-seam"};
    return line.starts_with(prefix);
}
...
auto format_runtime_indexed_cleanup_production_readiness_report(
    ...
    if (!state.diagnostic_source_text.empty()) {
        report << " source-text " << state.diagnostic_source_text;
    }
...
auto format_runtime_indexed_cleanup_production_readiness_blocker_report(
    ...
    for (auto index = std::size_t {0}; index < state.blockers.size(); ++index) {
        ...
        if (!blocker.source_text.empty()) {
            line << " source-text " << blocker.source_text;
        }
...
auto apply_runtime_indexed_cleanup_first_readiness_blocker(
    ...
    readiness_state.diagnostic_source_text = blocker.source_text;
}
...
auto runtime_indexed_cleanup_module_ir_production_readiness_state(
    ...
    for (auto& blocker : readiness_state.blockers) {
        if (blocker.source_text.empty()) {
            blocker.source_text = source_line_text(source_text, blocker.source_line);
        }
    }
...
void populate_lowering_emission_reports(
    ...
    result.runtime_indexed_member_cleanup_mutation_operation_plans =
        std::move(emission.runtime_indexed_member_cleanup_mutation_operation_plans);
    result.runtime_indexed_member_cleanup_mutation_operation_validations =
        std::move(emission.runtime_indexed_member_cleanup_mutation_operation_validations);
    result.runtime_indexed_member_cleanup_mutation_conflict_detections =
        std::move(emission.runtime_indexed_member_cleanup_mutation_conflict_detections);
